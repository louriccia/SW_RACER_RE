// Randomizer game-side integration: the hooks that arm the per-profile randomizer and apply its
// effects. The seed/config/sidecar logic lives in ../randomizer.{h,cpp}.
...
// so any other track dealt pod 2 silently grants nothing. NOPped while Track Favorites is active
// and restored via the patch journal when it is not.
...
// Uses the WORKING profile slot, not the save-image slot: the menu-time appliers run in the hangar
...
// swrRace_SaveProfile runs on profile creation and on every autosave, always with the current
// profile's name -- so arming here freezes the staged config for a new profile and loads the
// already-frozen one otherwise. Address-only.
...
// Runs the original InitAISettingsForTrack, then overwrites swrRace_AILevel / ai_spread with
// per-(profile, track) values from the AI sub-stream. The AI Speed menu scale is re-applied so that
// setting still has effect. Reimplemented function -> hook_replace.
void InitAISettingsForTrack_delta(swrObjJdge *judge) {
    hook_call_original(InitAISettingsForTrack, judge);

    // Never in multiplayer: these are shared globals and disarm is never called, so an SP-armed
    // profile would otherwise randomize AI in a later MP race.
    ...
    // InitAISettingsForTrack forces ai_spread = 2.0 when flag & 0x20 (reverse / special event);
    // preserve that or those races get the wrong AI field spacing.
    if ((judge->flag & 0x20) != 0)
        ai_spread = 2.0f;
}

// The course-selection menu maps slot -> track via g_aTrackIDs[circuit*7 + slot] while unlock
// gating keys off the SLOT index, so permuting within a circuit is safe. Called every frame on the
// track-select screen; idempotent because it always rebuilds from a one-time vanilla snapshot.
extern "C" void randomizer_apply_track_order(void) {
    ...
    // The trailing (empty / -1) slots are left untouched so a circuit never gains or loses one.
    ...
    if (cross) {
        // One global shuffle dealt back into the same-shaped slots: sizes are preserved but a track
        // can land in any circuit. Progression keys off slot position, not track id.
    ...
    // Fisher-Yates per circuit, keyed by circuit so each shuffles independently but stably.
    for (int c = 0; c < DEFAULT_NB_CIRCUIT_PER_TRACK; c++) {
        if (count[c] <= 1)
            continue;
...
// swrRacer_PodHandlingData[podId] is the stat block swrRace_Init copies into each pod at race
// start, so permuting it before the roster builds makes each pod drive like a different one (and
// the menu stat bars, reading the same table, match). Idempotent from a one-time snapshot.
...
void randomizer_apply_pod_handling(void) {
    ...
    randomizer_ensure_armed(live_profile_name());
    // Never in multiplayer: the table is shared and read by the MP roster builder, so an SP-armed
    // profile would leak its permutation into a later MP race and desync clients.
...
// The stat bars read swrRacer_PodHandlingData via swrObjHang_ComputeUpgradedStats, so the table has
// to be permuted here too -- BuildRosterSinglePlayer alone fixes the race, not the menu.
...
// Class-A: the new profile's starting money and/or unlocked pods, written into the working profile
// once at creation. TRACK unlocks are deliberately left at their defaults so race progression can
// never be soft-locked.
//
// Starting Pods unlocks EXACTLY N pods (slider, 1..23). swrRace_BuildPartMenuList normally
// force-ORs 6 base pods (0x22e01), which would floor the count at 6, so that OR is neutralized at
// hook install (see renderer_hook.cpp).
...
// Arms from the working profile name (already current here, unlike the save image this is about to
// sync) and applies the Class-A state before the copy to tgfd.dat, so it persists. Address-only.
...
// Winning a tournament track unlocks its FavoritePilot's pod (swrRace_ResultsMenu:
// podracers_unlocked |= 1 << FavoritePilot), so randomizing FavoritePilot randomizes the reward.
// Dealt from a shuffled 23-pod deck without replacement (with 25 tracks, only the last 2 can
// duplicate), from one sequential stream so the deal is a stable permutation per profile.
...
extern "C" void randomizer_apply_track_favorite(void) {
    ...
    for (int t = 0; t < NUM_TRACK_INFOS; t++) {
        ...
        // BOTH the vanilla table (read by the original unlock code) and the custom-tracks inflated
        // table (read by the menu display + reimplemented flow).
        g_aTrackInfos[t].FavoritePilot = val;
        g_aNewTrackInfos[t].FavoritePilot = val;
    }
}

// Free-play per-track mirror flag / lap count. Seeded only on a track or profile change, so the
// menu's own controls still override for the current visit. numLaps clamped 1-5.
extern "C" void randomizer_apply_track_race_settings(swrObjHang *hang) {
    ...
    // Keyed by planet-slot (planetTrackNumber + PlanetIdx*4), the SAME key swrObjJdge_F4_delta
    // uses, so a physical track gets identical mirror/laps in free play and tournament.
    int tid = (int) hang->track_index;
    int track = (tid >= 0 && tid < 25)
                    ? (g_aTrackInfos[tid].planetTrackNumber + g_aTrackInfos[tid].PlanetIdx * 4)
                    : tid;

    // Not every frame, so a manual adjustment survives until the player moves to another track.
...
// Tournament forces 3 laps / no mirror and bypasses the free-play seed. swrObjJdge_F4's 'Begn'
// sub-event latches num_laps from subEvents[9] and the mirror flag from subEvents[0xd], so both are
// rewritten BEFORE the original runs. Free play is left alone.
...
// The 12-value prize table (3 payout modes x 4 places) is written once by swrObjHang_Init into
// hang->winnings (@ +0x92), and vanilla's only per-circuit difference is a read-time linear scale
// (K1 - circuitIdx*K2). So the shuffle is keyed by the currently-viewed circuit (hang->circuitIdx)
// to give each one its own permutation. Idempotent from a one-time snapshot.
...
// upgradeInfos[42] (swrUpgradeInfo, 16B) is 7 categories x 6 slots; slot 0 of each (index % 6 == 0)
// is the base part, kept vanilla. `.cost` is read by swrRace_ComputeUpgradePrices.
